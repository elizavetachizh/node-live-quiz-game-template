...
export const handleCreateGame = (ws: WebSocket, data: CreateGameData): void => {
    ...
    const validQuestions = questions.filter(question => question.text && question.options && question.options.length === 4 && question.correctIndex >= 0 && question.correctIndex < question.options.length && question.timeLimitSec > 0);
